...
function errorMiddleware(
  ...
  if (response.headersSent) {
    logger.error("An Error has been Encountered, Headers are already sent", error);
    return next(error);
  }
  if (error instanceof HttpException) {
    logger.error("An Error has been Encountered!", error);
    response.status(error.statusCode);
    response.send(error);
  } else {
    logger.error("A Fatal Error has been Encountered!", error);
    response.status(500);
    response.send(getExceptionByStatusCode(500));
  }
}

export { errorMiddleware, exceptions, HttpException, getExceptionByStatusCode as code };
